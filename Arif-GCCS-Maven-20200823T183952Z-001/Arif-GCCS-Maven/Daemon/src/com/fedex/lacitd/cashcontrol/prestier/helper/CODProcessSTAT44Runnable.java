...
public class CODProcessSTAT44Runnable implements Runnable,Serializable
{
	/**
	 *  Serial Id
	 */
	private static final long serialVersionUID = 7461118063333405278L;
	...
	public CODProcessSTAT44Runnable()
	{
	}
	 ...
	 * 
	 * @param notProcessed
	 *            New value of property notProcessed.
	 */
	 ...
	 */
	public Collection processLastScans(java.util.Collection colAWBs)
	{// throws Exception{
		...
		extLoop: while (iterAWBs.hasNext())
		{
			...
			try
			{
				...
				colMasterList = csu
						.getCODScans(csVO.getAwbNbr(), csVO.getTinUniqId(),
								colNotProcessed, null, null, false);
				if (colMasterList != null && !colMasterList.isEmpty())
				{
					...
					while (iterMasterList.hasNext())
					{
						...
						if (ml.getEvent_sequence_nbr() != null
								&& Long.parseLong(ml.getEvent_sequence_nbr()
										.stringValue()) > Long.parseLong(csVO
										.getScanSeqNbr()))
						{
							String trackType = ml.getTrack_type() == null ? null
									: ml.getTrack_type().stringValue();
							String codeType = ml.getTrack_exception_code() == null ? null
									: ml.getTrack_exception_code()
											.stringValue();
							if (csVO.getTinUniqId() == null
									|| "".equals(csVO.getTinUniqId())) csVO
									.setTinUniqId(ml
											.getTracking_item_unique_id() == null ? null
											: ml.getTracking_item_unique_id()
													.stringValue());
							if (trackType != null)
							{
								if ("07".equals(trackType)
										&& "44".equals(codeType))
								{ // STAT44
									csVO.setCourierId(ml.getEmployee_number() == null ? null
											: ml.getEmployee_number()
													.stringValue());
									csVO.setScanLocationCd(ml
											.getTrack_location_code() == null ? null
											: ml.getTrack_location_code()
													.stringValue());
									csVO.setScanSeqNbr(ml
											.getEvent_sequence_nbr() == null ? null
											: ml.getEvent_sequence_nbr()
													.stringValue());
									String datepart = ml.getTrack_date() == null ? null
											: ml.getTrack_date().stringValue();
									String timepart = ml.getTrack_scan_time() == null ? null
											: ml.getTrack_scan_time()
													.stringValue();
									csVO.setScanDt(csu.parseDatesFromCosmos(
											datepart, timepart));
				...
				{
					Constants.logger.debug("MasterList doesn't exist. AWB: "
							+ csVO.getAwbNbr());
					colNotProcessed.add(new ScansProcessingError(csVO
							.getAwbNbr(),
							"app.messages.CannotFindDetailedScans", null, "",
							"", ""));
				}
			}
			catch (Exception e)
			{
				Constants.logger.debug("Scan Processing Error: "
						+ Utils.stackTraceToString(e));
				colNotProcessed.add(new ScansProcessingError(csVO.getAwbNbr(),
						"app.messages.UnknownError", null, "", "", ""));
			}
		}
