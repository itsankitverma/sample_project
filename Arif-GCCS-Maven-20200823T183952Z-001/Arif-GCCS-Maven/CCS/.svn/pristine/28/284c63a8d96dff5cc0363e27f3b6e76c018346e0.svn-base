 ...
 * @author  ccardenas
 */
public class CODScanProcessor implements java.io.Serializable {
    ...
    public Collection processLastScans(Collection awbs,Collection results,String locationCodePk,Collection validLocations,int scanLocalDecs,int scanUsdDecs) throws Exception{
    		...
    		if(awbs==null){
    			Constants.logger.debug("CODScanProcessor.java > processLastScans: awbs == null ");
    		}
    		if(results==null){
    			Constants.logger.debug("CODScanProcessor.java > processLastScans: results == null ");
    		}
    		if(locationCodePk==null){
    			Constants.logger.debug("CODScanProcessor.java > processLastScans: locationCodePk == null ");
    		}
    		if(validLocations==null){
    			Constants.logger.debug("CODScanProcessor.java > processLastScans: validLocations == null ");
    		}
    		
   			//Constants.logger.debug("CODScanProcessor.java > processLastScans: scanLocalDecs = "+scanLocalDecs);
   			//Constants.logger.debug("CODScanProcessor.java > processLastScans: scanUsdDecs = "+scanUsdDecs);
            ...
            if(!colAwbs.isEmpty()){
                ...
                for(int i=1;i<=threadsNumber;i++){
                    CODProcessLastScanRunnable rq=new CODProcessLastScanRunnable();                
                    rq.setErrors(errors);
