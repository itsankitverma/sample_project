 ...
 * @noinspection ALL
 */
public class RODProcessLastScanRunnable implements Runnable, java.io.Serializable {
    ...
    public RODProcessLastScanRunnable() {
    }
    ...
    /**
     * Process the scans from Cosmos for the ROD feature.
     * Returns a Collection with the AWB couldn't be processed properly.
     * 
     * @param     a Collection of RecChangesFromScans objects containing the AWBs
...
extLoop: while(iterAWBs.hasNext()){
            ...
            try{
                ...
                colMasterList=csu.getRODScans(recChanges.getRecAwbNumber(),recChanges.getRecTinUniqueId(),colNotProcessed,null,getValidLocations(),false);
                ...
                if(colMasterList!=null && !colMasterList.isEmpty()){
                    ...
                    while(iterMasterList.hasNext()){
                       ...
                       if(ml.getEvent_sequence_nbr()!=null && Long.parseLong(ml.getEvent_sequence_nbr().stringValue())>Long.parseLong((recChanges.getRecLastScan()==null?"0":recChanges.getRecLastScan()))){
                            ...
                            Date parsedScanDate = csu.parseDatesFromCosmos(datepart,timepart);
                            scanVO.setScanDt(parsedScanDate);

                            recChanges.getCosmosScans().add(scanVO);
                            recChanges.setLocationCd(scanVO.getScanLocationCd());
                           
                            /*
                            //==== PPM-45181 check if form id is TNT, if yes, find FDX 's associate TNT location                	                        	   
                            String formId=ml.getTracking_item_form_cd()==null?null:ml.getTracking_item_form_cd().stringValue();                	   
                     	    String locCd=recChanges.getLocationCd();                     	   
                     	    ...
                     	    if(formId!=null && locCd!=null && formId.length()==4 && formId.startsWith("11")){
                     		   ...
                     		   CosmosScanUtils cosmoUtil=new CosmosScanUtils();
                     		   locCd = cosmoUtil.getTNTLocation(locCd); 
                     		   cosmoUtil=null;
                     		   System.out.println("Derek after getTNTLocation locCd: " + locCd);
                               recChanges.setLocationCd(locCd);
                     	    }//end if                      	   
                     	   //==========================================================================                	   
                     	   */
                            ...
                            //System.out.println("RODProcessLastScanRunnable trackType: " + trackType);
                            //System.out.println("RODProcessLastScanRunnable codeType: " + codeType);
                                    ...
                                    if(("07".equals(trackType) && "44".equals(codeType)) || //STAT44
                                        ...
                                        ("31".equals(trackType) && "16".equals(codeType)) //DDEX =16
                                            ...
                                            if("20".equals(trackType)){
                                                ...
                                                //====Start has pod scan more tahn 7 days, deferred rod ============///
                                                if(recChanges.getLocationCd().equals("KULHQ") || recChanges.getLocationCd().equals("KULAX")){
	                                                Date todayDate = new Date();
	                                                Date podAfterDays = parsedScanDate;
	                                                Calendar c = Calendar.getInstance(); 
	                                                c.setTime(parsedScanDate); 
	                                                if(recChanges.getLocationCd().equals("KULHQ")){
	                                                	c.add(Calendar.DATE, 7); // scan day +7 days 
	                                                }else if(recChanges.getLocationCd().equals("KULAX")){
	                                                	c.add(Calendar.DATE, 12); // scan day +12 days
	                                                }
	                                                podAfterDays = c.getTime();
	                                                
	                                                //System.out.println("---------------RODProcessLastScanRunnable.java line 264  Loc_cd:"+recChanges.getLocationCd()+" podAfterDays:"+podAfterDays+"----todayDate:"+todayDate);
	                                                
	                                                if(todayDate.after(podAfterDays)){
	                                                    //System.out.println("---------------RODProcessLastScanRunnable.java line 271 Loc_cd:"+recChanges.getLocationCd()+" podAfterDays:"+podAfterDays+"---  todayDate.after(podAfterDays)");
	                                                	recChanges.setRecTrackingStatus(4);                                                	
	                                                }
                                                }//end if is Deferred ROD location
                                                //====End has pod scan more tahn 7 days, deferred rod ============///
                                            }else{
                                            ...
                                            if("31".equals(trackType) && "16".equals(codeType)){
                                                ...
                                                if((recChanges.getRecCashPayment()+recChanges.getRecOtherPayment())>0)
                                                    recChanges.setRecTrackingStatus(3);
                                                else if(recChanges.getRecTrackingStatus()==4){
                                                	//do nothing, already set to POD > 7 days status
                                                }else{
