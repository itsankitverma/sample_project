 ...
 * @author  ccardenas
 */
public class PendingFTCByCourierReport extends GenericReportServlet {
    
    /** Creates a new instance of PendingFTCRecByCourierReport */
    public PendingFTCByCourierReport() {
    }
    ...
    public InputStream getXMLInputStream(HttpServletRequest request, HttpServletResponse response) throws ServletException {
        ...
        try{
        	...
        	System.out.println("Inside try block in PendingFTCByCourierReport.java");
        	
            InitialContext c=new InitialContext();
            DataSource ds=(DataSource)c.lookup(Constants.CCSDataSource);
            conn=ds.getConnection();
            cs=conn.prepareCall("BEGIN SP_PENDING_FTC_BY_COUR_REPORT(?,?,?); END;");
