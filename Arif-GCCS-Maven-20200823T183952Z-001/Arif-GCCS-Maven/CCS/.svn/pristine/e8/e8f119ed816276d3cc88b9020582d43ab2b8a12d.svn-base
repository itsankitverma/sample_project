...
<script type="text/javascript">
    ...
    function changeStatus(cbo){
        ...
        if(status.options[status.selectedIndex].value==1){
            ...
            */
            ...
            var nTimes = <bean:write name="CODCourierCashRecapForm" property="paramNTimes" />;
            ...
            if(Math.abs(recAmount-recDex11Amount)>error||
			...
			if(otherPaymentType==3 && recOtherDocNumber.length>4){
               ...
               // alert("in CODSplitCurrency.jsp");
				...
				var params = "formName=COD&index="+index+"&billAccount="+billAccount;
				//window.open("<html:rewrite page="/tiles/COD/BillToAccount1.jsp" />?"+params,"BillToAccount","scrollbars=no,toolbar=no, directories=no, location=no, status=yes, menubar=no, resizable=yes,width="+w+", height="+h+",left="+leftpos+",top="+toppos);
				window.open("<html:rewrite page="/pages/COD_BillToAccount.jsp" />?"+params,"BillToAccount","scrollbars=no,toolbar=no, directories=no, location=no, status=yes, menubar=no, resizable=yes,width="+w+", height="+h+",left="+leftpos+",top="+toppos);
        }
    }
    ...
    function findIndex(name,after){
		...
		pop.document.write('var form  = self.opener.document.forms["CODCourierCashRecapForm"];')
    ...
    function changeCustomerNm(idx){
        var cust=eval("document.forms['CODCourierCashRecapForm'].elements['receivables["+idx+"].customerNm']");
        var custPrev=eval("document.forms['CODCourierCashRecapForm'].elements['receivables["+idx+"].customerNmPrev']");
    ...
    function changeRecAmt(idx){
        var recAmt=eval("document.forms['CODCourierCashRecapForm'].elements['receivables["+idx+"].recAmount']");
        var recAmtPrev=eval("document.forms['CODCourierCashRecapForm'].elements['receivables["+idx+"].recAmtPrev']");
    ...
    function changeReceiptNbr(idx){
        var rcptNbr=eval("document.forms['CODCourierCashRecapForm'].elements['receivables["+idx+"].rcptNbr']");
        var rcptNbrPrev=eval("document.forms['CODCourierCashRecapForm'].elements['receivables["+idx+"].rcptNbrPrev']");
	...
	function showScans(awb){
                ...
                var leftpos=(screen.width-w) / 2;
                var toppos=(screen.height-h) / 2;
                window.open("<html:rewrite page="/COD/COD_CourierCashRecap.do" />?action=showScans&awbNbr="+awb,"showScans","scrollbars=yes,innerHeight="+h+",innerWidth="+w+",toolbar=no, directories=no, location=no, status=yes, menubar=no, resizable=no, width="+w+", height="+h+",left="+leftpos+",top="+toppos);
	}

    function updateField(codeVal, index, otherComment)
    {
		if(codeVal.length > 0)
		{  document.forms['CODCourierCashRecapForm'].elements['imgComment'+index].src='<html:rewrite page="/images/hasComment.gif"/>';
		   document.forms['CODCourierCashRecapForm'].elements['imgComment'+index].alt=codeVal;
		}else{
			  document.forms['CODCourierCashRecapForm'].elements['imgComment'+index].src='<html:rewrite page="/images/hasNoComment.gif"/>';
			  document.forms['CODCourierCashRecapForm'].elements['imgComment'+index].alt="<bean:message key="app.messages.ViewAddComment"/>";
			 }
		document.forms['CODCourierCashRecapForm'].elements['receivables['+index+'].recComment'].value=codeVal;

		if(otherComment.length>0)
		{  		if(codeVal.length == 0)
			    {  document.forms['CODCourierCashRecapForm'].elements['imgComment'+index].src='<html:rewrite page="/images/hasComment.gif"/>';
				   document.forms['CODCourierCashRecapForm'].elements['imgComment'+index].alt="";
				}
		}else{
				if(codeVal.length == 0)
				{  document.forms['CODCourierCashRecapForm'].elements['imgComment'+index].src='<html:rewrite page="/images/hasNoComment.gif"/>';
		...
		document.forms['CODCourierCashRecapForm'].elements['receivables['+index+'].otherComment'].value=otherComment;
    }
/*
