...
<script>
    function assignThis(recId){
        document.forms['AddFTCReceivableForm'].recId.value=recId;
        document.forms['AddFTCReceivableForm'].action.value="ShowToAssign";
        document.forms['AddFTCReceivableForm'].submit();        
    }
    function createNew(){
        document.forms['AddFTCReceivableForm'].action.value="CreateNew";
        document.forms['AddFTCReceivableForm'].submit();        
    }

    function reassignAll(){
        document.forms['AddFTCReceivableForm'].action.value="ReassignReceivables";
        document.forms['AddFTCReceivableForm'].submit();
    }
...
<html:errors />
<nested:form action="/FTC/AddFTCReceivable.do">
    <input type="hidden" name="recId" value=""/>
    <input type="hidden" name="recEmployeeId" value="<c:out value='${AddFTCReceivableForm.recEmployeeId}'/>"/>
    <input type="hidden" name="currencyCode" value="<c:out value='${AddFTCReceivableForm.currencyCode}'/>"/>
    <input type="hidden" name="recAwbNumber" value="<c:out value='${AddFTCReceivableForm.recAwbNumber}'/>"/>
    <input type="hidden" name="locationCd" value="<c:out value='${AddFTCReceivableForm.locationCd}'/>"/>
...
<TABLE cellSpacing=1 cellPadding=0 width="100%" align=center bgColor="#000000">
  ...
  <TR>
    <TD bgcolor="#73148C" colSpan=8>
      <DIV align=center><FONT face="Arial" color="#FFFFFF" size=3><B><bean:message key="app.messages.ListOfFTC" /></B></FONT></DIV>
    </TD>
  </TR>
  <TR>
...
<nested:iterate property="awbs" id="rec" indexId="idx" type="com.fedex.lacitd.cashcontrol.biztier.common.ReceivablesByAwbVO">
  ...
  <TR>
    <TD align="right" bgColor="#ffffff" colSpan=4><INPUT type="button" onclick="reassignPayments();" value="<bean:message key="app.messages.AssignFTC" />"></TD>
    <TD align="left"  bgColor="#ffffff" colSpan=4><INPUT type="button" onclick="createNew();" value="<bean:message key="app.messages.CreateNewFTC" />"></TD>
  </TR>
</TBODY>
</TABLE>
</nested:form>