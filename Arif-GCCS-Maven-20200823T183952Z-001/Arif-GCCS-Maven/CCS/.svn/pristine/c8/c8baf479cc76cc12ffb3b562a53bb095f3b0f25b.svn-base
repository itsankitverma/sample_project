...
<script>
	...
	{	var codAmountValue;
		var ansChargeValue;
		var frm;
		frm = document.forms['CODAddAwbsForm'];
		
		codAmountValue = frm.elements['awbsList[0].codAmt'].value;
		ansChargeValue = frm.elements['awbsList[0].ancChargeAmt'].value;

		if(isNaN((codAmountValue - 0) + (ansChargeValue - 0)))
		{	frm.elements['awbsList[0].recAmount'].value = "<%= formatter.format(0)%>";
		}else
			{ frm.elements['awbsList[0].recAmount'].value = (codAmountValue - 0) + (ansChargeValue - 0);}
			
		return;
	}
    
	function addAwbs(){
        var form=document.forms['CODAddAwbsForm']; 
	...
	if(!(validaCODAWBNbr(form.elements['awbsList[0].recAwbNumber'].value)))
	{  return "<bean:message key="errors.messages.js.AwbInvalid"/>";}

        if(isNaN(Number(form.elements['awbsList[0].codAmt'].value)) || Number(form.elements['awbsList[0].codAmt'].value)<0)
		{ form.elements['awbsList[0].codAmt'].focus();
		  return "<bean:message key="app.messages.js.BadCODAmt"/>";	   
		}
...
<nested:form action="/COD/CODAddAwbs.do" focus="<%=elementFocus%>" >

<bean:define id="frm" name="CODAddAwbsForm" type="com.fedex.lacitd.cashcontrol.prestier.struts.form.CODAddAwbsForm"/>
            ...
            <td  width="84" bgcolor="#808080" height="2"><img src="<html:rewrite page="/images/spacer.gif" />" width="1" height="1"></td>
                  ...
                  <tr> 
                        <td bgcolor="#73148C" colspan="3" align="center"><b><font color="#FFFFFF" face="Arial" size="3">
                           <bean:message key="app.AddCODAwbs" />
                        </font></b></td>
                  </tr>
                  <tr>
		   ...
		   <td  width="100%" bgcolor="#FFFF9C" align="center"> 
             ...
             <table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#000000" >
                  ...
                  <tr>
                    ...
                    <td width="17%" height="12" bgcolor="#73148C" align="center"><font face="Arial" size="1" color="#FFFFFF"><b><bean:message key="app.messages.codAmount" /></b></font></td>
                  ...
                  <nested:iterate property="awbsList" id="awbsList" indexId="idx" type="com.fedex.lacitd.cashcontrol.biztier.common.COD_RecChangesFromScans">
				  <nested:equal name="idx" value="0">
					  <tr>
						<% COD_RecChangesFromScans sc= (COD_RecChangesFromScans)(frm.getAwbsList().get(0));%>
						<td width="17%" height="23" bgcolor="#FFFF9C"><div align="center"><nested:text property="recAwbNumber" onkeypress="handlePress(event,document.forms['CODAddAwbsForm'].elements['awbsList[0].awbNumber'])" size="12" value="" style="font-family: Verdana, Arial, Helvetica, sans-serif; font-size: 8 pt"/></div></td>
						<td width="17%" height="23" bgcolor="#FFFF9C"><div align="center"><nested:text property="codAmt" value="<%= formatter.format(sc.getCodAmt())%>" onblur="JavaScript:setRecAmount()" onkeypress="handlePress(event,document.forms['CODAddAwbsForm'].elements['awbsList[0].codAmt'])" size="12" style="font-family: Verdana, Arial, Helvetica, sans-serif; font-size: 8 pt"/></div></td>
                  ...
                  <tr>
                    ...
                    <td width="17%" height="18" bgcolor="#FFFF9C" align="right"><font face="Arial" size="1"><nested:write property="codAmt" formatKey="app.format.NumberFormat"/><nested:hidden property="codAmt" /></font></td>
