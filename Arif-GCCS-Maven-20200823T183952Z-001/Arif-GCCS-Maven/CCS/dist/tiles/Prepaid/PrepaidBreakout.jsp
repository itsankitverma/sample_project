...
<script>
	function save(){
        var form=document.forms['PrepaidBreakoutForm'];	
        var parentForm=opener.document.forms['PRPCourierCashRecapForm'];
        
        parentForm.elements['awbs[<bean:write name="PrepaidBreakoutForm" property="parentIndex" />].surChargesTotal'].value=form.elements['totalCollected'].value;
        if(form.elements['totalCollected'].value==Number(0)){
        	opener.document.images['surchrgImg<bean:write name="PrepaidBreakoutForm" property="parentIndex" />'].src="<html:rewrite page="/images/moneyGray.gif"/>";
        	form.submit();
        }else{
	        if(Number(form.elements['totalCollected'].value)!=Number(form.elements['parentTotal'].value)){
	        	opener.document.images['surchrgImg<bean:write name="PrepaidBreakoutForm" property="parentIndex" />'].src="<html:rewrite page="/images/moneyRed.gif"/>";
	        	if(confirm("<bean:message key="app.messages.js.TotalSurchNotMatch" />")){
	 				form.submit();       	
	        	}
	        }else{
	        	opener.document.images['surchrgImg<bean:write name="PrepaidBreakoutForm" property="parentIndex" />'].src="<html:rewrite page="/images/moneyGreen.gif"/>";
	        	form.submit();
	        }
	    }    
	}
...
<html:errors />
<bean:define id="frm" name="PrepaidBreakoutForm" toScope="page" type="com.fedex.lacitd.cashcontrol.prestier.struts.form.PrepaidBreakoutForm" />
<nested:form action="/Prepaid/PrepaidBreakout.do" >
             ...
             <table width="275" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#000000" height="129" >
                  <tr>
                    <td colspan="2" height="19" bgcolor="#73148C" align="center" width="367">
                      <p align="center"><b>
                      <font face="Arial" size="2" color="#FFFFFF"><bean:message key="app.messages.FreightPaymentBreakout" /></font></b></td>
                  </tr>
                  <tr>
