/**
 * RODCourierCashRecapAction.java
 *
 * 
 */
...
import com.fedex.lacitd.cashcontrol.prestier.helper.RODScanProcessor;
import com.fedex.lacitd.cashcontrol.prestier.helper.ShowScans;
import com.fedex.lacitd.cashcontrol.prestier.struts.form.RODCourierCashRecapForm;
 ...
 * @author ccardenas
 */
public class RODCourierCashRecapAction extends Action implements java.io.Serializable {
    /**
    ...
    public ActionForward execute(ActionMapping mapping,
        ...
        request.setAttribute("RODCourierCashRecapForm", copyForm);
    ...
    private ActionForward splitCurrency(ActionMapping mapping,
        ...
        RODBizDelegate rodBiz = new RODBizDelegate();
        RODCourierCashRecapForm ccrForm = (RODCourierCashRecapForm) form;
        ...
        CourierCashRecapTableVO courierCashRecap = null;
        Iterator itRec = ccrForm.getReceivables().iterator();

        while (itRec.hasNext()) {
            courierCashRecap = (CourierCashRecapTableVO) itRec.next();
        ...
        rodBiz.splitCurrency(receivablesSelected, scVO.getCurrencyCode(), exchangeRate, amountToChange);
     ...
     * request for the ROD Details screen page.
     *
     * @param Struts action mapping object
     * @param Struts action form object
     * @param http   request object
    ...
    private ActionForward show(ActionMapping mapping,
        ...
        try {
            ...
            /* Start- For Mcd start for ROD Detail screen*/
        	long startTime = System.currentTimeMillis() ;
            try {
            	System.out.println("In Mcd method in Rod detail screen");
                // Beginning of request. Inform McD
                MCD.processEvent("server",
                    "GCCS",
                    "RODCourierCashRecapAction",
                ...
                //AppLogger.error("McD Exception while processing GCCS.RodDetailScreen", e) ;
                System.out.println("McD Exception while processing GCCS.RodDetailScreen") ;
            	e.printStackTrace();
            }
            /* End- For Mcd start for ROD Detail screen*/
            
            
            RODBizDelegate biz = new RODBizDelegate();
            //set supported and currently being used currencies for ROD
            CommonOpsBizDelegate bizComm = new CommonOpsBizDelegate();
            Collection colUsedCurr = biz.getRODUsedCurrencies(ep.getLocationCd(), ccrForm.getCourier());
            ...
            if (request.getAttribute("updated") == null && //if still it was not updated
                    ...
                    && !request.getParameter("action").equals("toROD")) {
                updateInformation(mapping, form, request, response);
            }
           ...
           /* Start- For Mcd End for ROD Detail screen*/            
            try {
                MCD.processEvent("server",
	                "GCCS",
	                "RODCourierCashRecapAction",
            ...
            /* End- For Mcd End for ROD Detail screen*/
            ...
            //====== The following is to remove "In Dispute" status for non Deferred ROD loc cd ====////
            if(!ep.getLocationCd().equals("KULHQ")&&!ep.getLocationCd().equals("KULAX")){
	            Iterator itr = all_rec_status.iterator();
	            RecStatusVO rsVO = new RecStatusVO();
	            while(itr.hasNext()){             
	            	rsVO = (RecStatusVO)itr.next();
		            if(rsVO.getDescription().equals("In Dispute")){
		            	itr.remove();
		            	break;
		            }//end if
	            }//end while
            ...
            ////////////////////////////////////
            Collection paymentTypes = new PaymentTypeController().getPaymentTypeByRodLocation(ep.getLocationCd());
            ...
            /////////////////////////////////////////////////////
            int splittes = new CommonOpsBizDelegate().getSplitCountTable(ep.getLocationCd(), ccrForm.getCourier(), "ROD");
            if (splittes > 0)
            ...
            request.setAttribute("RODCourierCashRecapForm", ccrForm);
    ...
    private ActionForward updateInformation(ActionMapping mapping,
        ...
        while (iterRec.hasNext()) {
            CourierCashRecapTableVO rec = (CourierCashRecapTableVO) iterRec.next();
            if (!rec.isModified()) {
                iterRec.remove();
            } else {
                EmployeeProfile ep = (EmployeeProfile) request.getSession().getAttribute("userProfile");
    ...
    private ActionForward reassignPayments(ActionMapping mapping,
        ...
        while (iterRec.hasNext()) {
            ...
            if (rec.isSelected()) {
                ReceivablesVO rVO = rc.getReceivables(new Integer(rec.getRecId()));
    ...
    private ActionForward closeEmployee(ActionMapping mapping,
            ...
            throws Exception {
        updateInformation(mapping, form, request, response);
        System.out.println("here we are at the very first step");
        RODCourierCashRecapForm ccrForm = (RODCourierCashRecapForm) form;
        System.out.println("here we are at the very 2 step");
        Collection colCourier = new ArrayList();
        colCourier.add(ccrForm.getCourier());
        System.out.println("here we are at the very 3 step");
        EmployeeProfile ep = (EmployeeProfile) request.getSession().getAttribute("userProfile");
        System.out.println("here we are at the very 4 step");
        CommonOpsBizDelegate bizComm = new CommonOpsBizDelegate();
    ...
    public ActionForward processScans(ActionMapping mapping,
        ...
        try {
            ...
            System.out.println("Before Mcd in Process SCAN TESTT");
            ...
            try {
                // Beginning of request. Inform McD
            	System.out.println("Mcd in Rod Process Scans");
                MCD.processEvent("server",
                    "GCCS",
                    "RODCourierCashRecapAction",
                    //AppUtils.getHostname(),
                    ...
                    null,
                    "OR",
                    "processScansRODCourierCashRecap",
                    null,
                    null,
                    -1,
                    null);
            } catch (Exception e) {
                //AppLogger.error("McD Exception while processing processScansRODCourierCashRecap", e) ;
                System.out.println("McD Exception while processing GCCS.processScansRODCourierCashRecap") ;
            	e.printStackTrace();
            }
    ...
    private ActionForward quickApplyStatus(ActionMapping mapping,
        ...
        RODCourierCashRecapForm ccrForm = null;
        if (form == null) {
            ccrForm = new RODCourierCashRecapForm();
        } else {
            ccrForm = (RODCourierCashRecapForm) form;
        }
    ...
    */
     ...
     * @throws Exception
     */
    private void calcTotals(RODCourierCashRecapForm ccrForm,
                            String locationCd,
                            String employeeId,
        ...
        if (ccrForm.isDualCurrency() && "split".equals(currentCurrencyCd)) {
            ...
            RODBizDelegate delegate = new RODBizDelegate();
            TotalSummaryVO totalSummaryVO = delegate.getRODDetailsTotal(locationCd, employeeId, currentCurrencyCd, pageNumber, rowsByPage, sortColumn, sortDirection);
     ...
     * @throws Exception
     */
    private void calcTotalsSplit(RODCourierCashRecapForm ccrForm,
                                 String locationCd,
                                 String employeeId,
                                 Integer pageNumber,
                                 Integer rowsByPage,
                                 ...
                                 String sortDirection) throws Exception {
        RODBizDelegate delegate = new RODBizDelegate();
        TotalSummaryVO totalSummaryVO = delegate.getRODDetailsSplitTotal(locationCd, employeeId, pageNumber, rowsByPage, sortColumn, sortDirection);
        ccrForm.setCashTotal(totalSummaryVO.getCashTotal().doubleValue());
        ccrForm.setDepositTotal(totalSummaryVO.getDepositTotal().doubleValue());
        ccrForm.setCreditCardTotal(totalSummaryVO.getCreditCardTotal().doubleValue());
        ccrForm.setCheckTotal(totalSummaryVO.getCheckTotal().doubleValue());
    ...
    private ActionForward showScans(ActionMapping mapping,
        ...
        try {
            ...
            List colResults = (List) ss.getRODScans(request.getParameter("awbNbr"), ep.getLocationCd());
