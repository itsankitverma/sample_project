 ...
 */
public class RODAddAwbsAction extends Action implements Serializable {
    
    /** Creates a new instance of RODAddAwbsAction */
    public RODAddAwbsAction() {
    }
    ...
    throws Exception
    {
         RODAddAwbsForm frm=(RODAddAwbsForm)form;
         if(frm==null)  frm=new RODAddAwbsForm();
         
         frm.getAwbsList().add(0,new RecChangesFromScans());        
         
         request.setAttribute("RODAddAwbsForm",frm);
    ...
    throws Exception {
        ...
        // This code is used to get the Cosmos data to complete the information of the ROD payments
           try{
                ...
                if(frm.getCosmosScansEnabled()){
                   np=sp.processLastScans(awbs,results,ep.getLocationCd(),comBiz.getLocationCdByCountry(ep.getCountryCd()),ep.getScansLocalDecs(),ep.getScansUsdDecs());
                        ...
                        while(awbsIt.hasNext()){
                              RecChangesFromScans recChanges = (RecChangesFromScans)awbsIt.next();
