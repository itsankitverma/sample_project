...
<nested:form action="/COD/AddCODReceivable.do" method="POST">
   ...
   function validateEnteredAwb(obj){
    ...
    function validate(){
        ...
        while(eval("form.elements['awbs["+index+"].recAwbNumber']")){
            if(validaCODAWBNbr(eval("form.elements['awbs["+index+"].recAwbNumber']").value)) return "";
            index++
        }
...
<nested:hidden property="currencyCode" />
  ...
  <TR>
    <TD bgcolor="#73148C" colSpan=10>
      <DIV align=center><FONT face="Arial" color="#FFFFFF" size=3><B><bean:message key="app.messages.FindCOD" /></B></FONT></DIV>
    </TD>
  </TR>
  <tr>
<nested:iterate property="awbs" indexId="idx">
  ...
  <tr>
      <TD align=middle colSpan=10>
        <INPUT type=button onclick="submitForm();" value="<bean:message key="app.messages.FindCOD" />">
    </TD>
  </TR>
</TBODY>
</TABLE>
