...
<script>
    function assignThis(recId){
        document.forms['AddCODReceivableForm'].recId.value=recId;
        document.forms['AddCODReceivableForm'].action.value="ShowToAssign";
        document.forms['AddCODReceivableForm'].submit();        
    }
    function createNew(){
        document.forms['AddCODReceivableForm'].action.value="CreateNew";
        document.forms['AddCODReceivableForm'].submit();        
    }

    function reassignAll(){
        document.forms['AddCODReceivableForm'].action.value="ReassignReceivables";
        document.forms['AddCODReceivableForm'].submit();
    }

    function reassignPayments(){
        var form=document.forms['AddCODReceivableForm'];
        var w=400;
        var h=200;
        var index=Number(0);
        var count=Number(0);
...
<html:errors />
<nested:form action="/COD/AddCODReceivable.do">
    <input type="hidden" name="recId" value=""/>
    <input type="hidden" name="recEmployeeId" value="<c:out value='${AddCODReceivableForm.recEmployeeId}'/>"/>
    <input type="hidden" name="currencyCode" value="<c:out value='${AddCODReceivableForm.currencyCode}'/>"/>
    <input type="hidden" name="recAwbNumber" value="<c:out value='${AddCODReceivableForm.recAwbNumber}'/>"/>
    <input type="hidden" name="locationCd" value="<c:out value='${AddCODReceivableForm.locationCd}'/>"/>
...
<TABLE cellSpacing=1 cellPadding=0 width="100%" align=center bgColor="#000000">
  ...
  <TR>
    <TD bgcolor="#73148C" colSpan=8>
      <DIV align=center><FONT face="Arial" color="#FFFFFF" size=3><B><bean:message key="app.messages.ListOfCOD" /></B></FONT></DIV>
    </TD>
  </TR>
  <TR>
...
<nested:iterate property="awbs" id="rec" indexId="idx" type="com.fedex.lacitd.cashcontrol.biztier.common.ReceivablesByAwbVO">
  ...
  <TR>
    <TD align="right" bgColor="#ffffff" colSpan=4><INPUT type="button" onclick="reassignPayments();" value="<bean:message key="app.messages.AssignCOD" />"></TD>
    <TD align="left"  bgColor="#ffffff" colSpan=4><INPUT type="button" onclick="createNew();" value="<bean:message key="app.messages.CreateNewCOD" />"></TD>
  </TR>
</TBODY>
</TABLE>
</nested:form>